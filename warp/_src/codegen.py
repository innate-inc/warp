...
class Adjoint:
    ...
    def _try_lower_array_slot_write(adj, lhs, rhs):
        ...
        adj.add_reverse(
            ...
            # array elements live in device memory on Metal (WP_DEVICE is empty elsewhere)
            f"[&](auto WP_DEVICE& _e) -> auto WP_DEVICE& {{ return _e{access_cpp}; }}, {array_indices_cpp});"
        )
    ...
    def _store_subscript(adj, lhs, target, indices, rhs):
        ...
        if is_array(target_type):
            ...
            if is_reference(target.type):
                ...
                if is_reference(target.type) and target.address_space() == "WP_DEVICE":
                    attr.device_memory = True  # a component of an array element stays in device memory
                adj.add_builtin_call("store", [attr, rhs])
    ...
    def _store_attribute(adj, lhs, aggregate, rhs):
        ...
        if type_is_vector(aggregate_type) or type_is_quaternion(aggregate_type):
            ...
            if is_reference(aggregate.type):
                ...
                if is_reference(aggregate.type) and aggregate.address_space() == "WP_DEVICE":
                    attr.device_memory = True  # a component of an array element stays in device memory
                adj.add_builtin_call("store", [attr, rhs])
            else:
                if adj.builder_options.get("enable_vector_component_overwrites", False):
...
def codegen_func(adj, c_func_name: str, device="cpu", options=None, forward_only=False, reverse_only=False):
    ...
    inline_attr = ""
    if device in ("cpu", "metal") and _metal_force_inline(c_func_name, forward_body):
...
def codegen_snippet(adj, name, snippet, adj_snippet, replay_snippet, forward_only=False, reverse_only=False):
    ...
    reverse_template = cuda_reverse_function_template
    inline_attr = ""  # native snippets use the CUDA-style templates, which have no inline slot
...
def codegen_module(kernel, device, options):
    ...
    # The CPU launch path follows the block size the module is compiled for (WP_TILE_BLOCK_DIM in the
    # module header), not a per-kernel "block_dim" option: kernels built by code that is not aware of
    # cooperative CPU blocks, such as the stock warp.fem of 1.17, carry the CUDA default of 256 there.
    module_block_dim = options["block_dim"]
