...
class Runtime:
    def __init__(self):
        ...
        if os.path.exists(llvm_lib):
            ...
            # The 1.17 warp-clang library, which an overlay package takes from the stock wheel, has no
            # CPU block runtime hook. Cooperative CPU blocks then stay unavailable (block_dim 1 on CPU).
            self.cpu_blocks_available = hasattr(self.llvm, "wp_llvm_set_cpu_block_runtime")
            if self.cpu_blocks_available:
...
def _resolve_launch_block_dim(device: Device, block_dim: int | None) -> int:
    ...
    # warp.config has no enable_cpu_blocks in 1.17: the option arrives with Warp 1.18
    if block_dim == 1 or not getattr(warp.config, "enable_cpu_blocks", False):
        return 1
    if not getattr(runtime, "cpu_blocks_available", False):
        raise NotImplementedError("Cooperative CPU blocks need a warp-clang library with CPU block support")
